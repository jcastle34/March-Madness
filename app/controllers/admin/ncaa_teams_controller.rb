class Admin::NcaaTeamsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @ncaa_team.save
        format.html { redirect_to(admin_ncaa_teams_url, :notice => 'Ncaa team was successfully created.') }
  ...
  def update
    ...
    respond_to do |format|
      if @ncaa_team.update_attributes(params[:ncaa_team])
        format.html { redirect_to(admin_ncaa_teams_url, :notice => 'Ncaa team was successfully updated.') }
        format.xml  { head :ok }
      else
