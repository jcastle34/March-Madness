  ...
  attr_accessible :mm_team_id, :ncaa_player_id
	...
	def self.ownership_for_player(player_id)
  	...
  	def self.get_preferred_players_ownership
    	...
    	MmTeamPreferredPlayer.find_by_sql("select count(np.id) as total_owned, np.first_name, np.last_name, np.position, np.ppg_avg, nt.school, be.seed, round(count(np.id) / #{mm_team_total} * 100, 2) as percentage_owned, nt.is_active from mm_team_preferred_players pp 
